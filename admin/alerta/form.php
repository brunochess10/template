...
//--------------------------------------------------------
		...
		$real_fields ="A.ID, A.TITULO, " .
					           "A.TEXTO, A.PUBLICAR";

		if  ($mode == "edit" || $mode == "detail" ){
			$key = request("key");
			$sql = "select $real_fields from site_alerta A where A.". $modules["alerta"]["edit_key"] . "='$key' LIMIT 0,1";
		} else if ( $mode == "insert") {
			$sql = "select  $real_fields from site_alerta A limit 0,1";
		}
		...
		$params_item[$index]["caption"] = "Código do alerta";
		...
		unset($i);
		$inputs = get_inputs( $ALERTA, $params_item, $mode, $sufix_alerta_new, $sufix_alerta_old);
?>
<!--
...
<?php
	...
	$CKEditor->replace("TEXTO_alerta_nv_0");
?>