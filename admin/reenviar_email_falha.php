...
<thead>
	<tr>
		...
		<th class="centro">REENVIAR EMAIL COM FALHA</th>
...
<fieldset class="destaquefild" style="display:none;" id="enviar_boletim">
	...
	<div id="espera">
		<b>Aguarde enquanto o reenvio do e-mail é processado...</b>
	</div>
...
<table class="formeditar">
	<tr>
		<td>
			...
			*Atenção: Nós tentaremos reenviar os e-mails que deram problemas no envio dos boletins.	
			<!-- <form>
			...
			-->
			<br/>
			<input type="button" value="Renviar e-mail agora" class="botaofild" id="disparar_email"><br/><br/>
			<!--<input type="button" value="Agendar envio do boletim" class="botaofild" id="agendar_email"><br/>-->
			
			<div id="msg_enviado" style="display:none;">
				<strong style='color:red'>Atenção! Você já tentou reenviar novamente todos os e-mails com falha.</strong>
			</div>
			...
			<div id="email" style="display:block;">
				<?php
					...
					###########################################################################
					# Esse select vai pegar todos os usuários que tiveram falha no envio do 
					# Boletim e vai inserir na tabela de reenvio do boletim
					###########################################################################
					$sql = "SELECT * FROM log_mail WHERE mail_id = '$mail_id' and status='2'";
					//echo $sql;
					$REENVIO = new QUERY($DATABASE,$sql);
					
					###########################################################################
					# Insere os boletins com problema nessa tabela de reenvio dos boletins
					# Quando o campo reenvio é 0 não foi tentado reenviar.
					###########################################################################
					
					while ($REENVIO->NEXT()){
						$sql = "SELECT * FROM reenvio_mail WHERE mail_id='".$REENVIO->BYNAME("mail_id")."' AND mailing_id='".$REENVIO->BYNAME("mailing_id")."'";
						$VERIFICA_REENVIO = new QUERY($DATABASE,$sql);
						#######################################################################
						# Se o e-mail não estiver na tabela de reenvio ele tenta enviar novamente
						#######################################################################
						if (!$VERIFICA_REENVIO->NEXT()){
							$sql = "INSERT INTO reenvio_mail (mail_id,mailing_id,reenviado) VALUES ('".$REENVIO->BYNAME("mail_id")."','".$REENVIO->BYNAME("mailing_id")."','0')";	
							$INSERE_REENVIO = new QUERY($DATABASE,$sql);
							$INSERE_REENVIO->FREE();
						}	
					}
					
					###############################################################################################################
					# Pega todos os e-mail que não foram disparados por algum problema para saber o total de e-mails.
					###############################################################################################################
					$sql = "SELECT * FROM reenvio_mail where mail_id='$mail_id'";
					$USERS = new QUERY($DATABASE,$sql);
					
					###############################################################################################################
					# Pega os e-mails que ainda não foram enviados por algum problema.
					###############################################################################################################
					$sql = "SELECT * FROM reenvio_mail WHERE reenviado <> '0' and mail_id = '$mail_id' ";
...
<?php
	if ($USERS->NUMROWS()==$NAO_ENVIADO->NUMROWS()){
?>
	<script>
	...
	function getSecs(){
		...
		if (sSecs==0){
			...
			*/
			$.ajax({
					type: "POST",
					url:"robo_reenvia_email_boletim.php",
