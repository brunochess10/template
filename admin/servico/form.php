...
//--------------------------------------------------------
		...
		$real_fields ="S.serv_id, S.titulo, " .
					           "S.texto, S.prioridade, S.publicar";

		if  ($mode == "edit" || $mode == "detail" ){
			$key = request("key");
			$sql = "select $real_fields from site_servico S where S.". $modules["servico"]["edit_key"] . "='$key' LIMIT 0,1";
		} else if ( $mode == "insert") {
			$sql = "select  $real_fields from site_servico S limit 0,1";
		}
		$SERVICO = new QUERY($DATABASE,$sql);	                
		$SERVICO->NEXT();
		...
		$params_item[$index]["name"] = "serv_id";
		...
		unset($i);
		$inputs = get_inputs( $SERVICO, $params_item, $mode, $sufix_servico_new, $sufix_servico_old);
?>
<!--
...
<?php
	...
	$CKEditor->replace("texto_servico_nv_0");
?>