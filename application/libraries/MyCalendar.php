...
class Calendar
{
    ...
    */
    function getMonthHTML($m, $y, $showYear = 1)
    {
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
        	    $class = ($year == $today["year"] && $month == $today["mon"] && $d == $today["mday"]) ? " class=\"hoje\" " : "";
    	        $s .= "<td $class >";       
    	        if ($d > 0 && $d <= $daysInMonth)
    	        {
    	            $link = $this->getDateLink($d, $month, $year);
					
    	            $s .= (($link == "") ? $span_open. $d. $span_close : "$span_open<a href=\"$link&amp;day=$d#$d\">$d</a>$span_close");
    	        }
    	        else
    	        {
    	            $s .= "&nbsp;";
    ...
    */
    function getYearHTML($year)
    {
        ...
        $s .= "<td class=\"phpcalendarHeader\" >" . (($this->startMonth > 1) ? $year . " - " . ($year + 1) : $year) ."</td>\n";
...
class MyCalendar extends Calendar
{
	...
	var $filter;
	var $uf;
	var $pagina_exibicao; // caso for através de ajax verifica qual é a página para montar os links
	
	
	function set_MyCalendar($days,$filter=0,$uf="",$pagina_exibicao=""){
		$this->days = $days;
		$this->filter = $filter;
		$this->uf = $uf;	
		$this->pagina_exibicao = $pagina_exibicao;
	}
    ...
    function getCalendarLink($month,$year)
    {
		...
		if ($this->pagina_exibicao==""){
        	$s = getenv('SCRIPT_NAME');
		}
		else{
			$s = $this->pagina_exibicao;
		}
		return "$s?month=$month&amp;year=$year&amp;filtro=$this->filter&amp;uf=$this->uf";
    }
	...
	function getDateLink($day, $month, $year)
    {
        ...
        $link = "";
        //print_r($this->days);
        //echo count($this->days);
		for( $i = 0; $i < count($this->days); $i++){
                 if (isset($this->days[ $i ])){   
                    if ($day == $this->days[ $i ]){
                        $link = $this->pagina_exibicao."agenda/?month=$month&amp;year=$year&amp;filtro=$this->filter&amp;uf=$this->uf";
                        break;
                    }
                 }
		}
